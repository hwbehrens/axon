...
- `persistence.rs`: the transaction worker (snapshot → lock-free save → delta apply) shared by every persistent edit.
...
- Peer-store I/O never runs under the state lock: each persistent edit is ONE serialized transaction — the save gate is held across build, save, and apply, so no races, retries, heal paths, or generation counters exist (DEC-021/022/023/024); the save gate orders gate -> state lock only.
