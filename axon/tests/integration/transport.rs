...
fn envelope_roundtrip_all_kinds() {
    let a = AgentId::parse("ed25519.a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4").unwrap();
    let b = AgentId::parse("ed25519.f6e5d4c3b2a1f6e5d4c3b2a1f6e5d4c3").unwrap();
    ...
    for (kind, payload) in payloads {
        let env = Envelope::new(a.clone(), b.clone(), kind.clone(), payload.clone());
        let encoded = encode(&env).unwrap();
        let decoded = decode(&encoded).unwrap();
        assert_eq!(decoded.kind, kind, "kind mismatch for {kind}");
        assert_eq!(decoded.from, Some(a.clone()));
        assert_eq!(decoded.to, Some(b.clone()));
    }
}
